...
public class VehicleDisplayWidget extends WidgetHudWidget<HudWidgetConfig> implements
  ...
  public void render(Stack stack, MutableMouse mouse, float partialTicks, boolean isEditorContext, HudSize size) {
    drawLine(stack, size, Math.max(0, Math.min(SPEED_NEEDLE_MAX, this.speed * SPEED_NEEDLE_UNIT)), VehicleDisplayTexture.SPEED_NEEDLE.getIcon(this.nightMode), 4F);
    drawLine(stack, size, Math.min(0, Math.max(FUEL_NEEDLE_MAX, this.fuel * FUEL_NEEDLE_UNIT)), VehicleDisplayTexture.FUEL_NEEDLE.getIcon(this.nightMode), 4F);
    ...
    if (this.limiterSpeed >= 10) {
      ...
      drawLine(stack, size, Math.max(0, Math.min(SPEED_NEEDLE_MAX, this.limiterSpeed * SPEED_NEEDLE_UNIT)), icon, 3F);
    }
  ...
  public boolean isVisibleInGame() {
    return this.germanMinerAddon.enabled() && !Laby.references().chatAccessor().isChatOpen() && this.show;
  }
  ...
  public void handle(VehicleDisplayPacket packet) {
    ...
    if (packet.getNightModus() != null) {
      this.displayContent.getBackground().icon().set(VehicleDisplayTexture.SPEEDOMETER_FUEL.getIcon(packet.getNightModus() != null && packet.getNightModus()));
  ...
  public void drawInfo(VehicleDisplayPacket packet) {
    ...
    if ("STARTING".equals(packet.getEngineState())) {
      ...
      info = Component.translatable(String.format("germanmineraddon.hudWidget.%s.gear", super.getId()),
          Component.text(packet.getGearPosition().charAt(0)));
    } else if (packet.getFlightHeight() != null) {
      info = Component.translatable(String.format("germanmineraddon.hudWidget.%s.height", super.getId()),
  ...
  public void drawSpeedLimiter(VehicleDisplayPacket packet) {
    if (packet.getLimiterSpeed() >= 10) {
      Icon icon = packet.getLimiterActive() != null && packet.getLimiterActive() ?
          VehicleDisplayTexture.SPEED_LIMITER_ACTIVE.getIcon(packet.getNightModus() != null && packet.getNightModus()) :
          VehicleDisplayTexture.SPEED_LIMITER_READY.getIcon(packet.getNightModus() != null && packet.getNightModus());
  ...
  public void drawWarning(VehicleDisplayPacket packet) {
    if (packet.getDamageState() >= 1) {
      Icon icon = packet.getDamageState() == 1 ?
          VehicleDisplayTexture.WARNING_YELLOW.getIcon(packet.getNightModus() != null && packet.getNightModus()) :
          VehicleDisplayTexture.WARNING_RED.getIcon(packet.getNightModus() != null && packet.getNightModus());
  ...
  public void sendInfo() {
    this.germanMinerAddon.sendPacket(new VehicleDisplayPacket(this.germanMinerAddon.enabled() && super.isEnabled()));
  }
}
